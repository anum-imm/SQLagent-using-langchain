...
# ### On Errors:
# ignore the case sensitiveness whenever u receive some information , makesure it gets it properly
# If a query fails due to syntax or column errors, examine the error, fix the query, and retry.
# Retrieve the schema for the most relevant table(s) or view(s) using `sql_db_schema`. 
# When calling `sql_db_schema`, pass only a comma-separated list of table names (e.g., `emp, dept`) with no extra explanation or text.
